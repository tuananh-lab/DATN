...
static SmolRTSP_Request options_req, describe_req, setup_req, play_req,pause_req,
    teardown_req, abracadabra_req;

typedef struct {
    bool options_completed, describe_completed, setup_completed, play_completed,pause_completed,
...
Client_pause(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
    ...
    assert(req == &pause_req);
    smolrtsp_header(ctx, HEADER_TEST_METHOD, "pause");
    const ssize_t ret = smolrtsp_respond_ok(ctx);
    assert(ret > 0);
}
...
TEST dispatch(void) {
    options_req.cseq = describe_req.cseq = setup_req.cseq = play_req.cseq = pause_req.cseq =
    ...
    pause_req.start_line.method = SMOLRTSP_METHOD_PAUSE;
