...
#define SmolRTSP_Controller_IFACE                                              \
     ...
     */                                                                        \
    vfunc99(                                                                   \
        void, pause, VSelf99, SmolRTSP_Context *ctx,                           \
        const SmolRTSP_Request *req){
            Client_pause(ctx,req);
        }                                           \
    /*                                                                         \
     * Handle `PAUSE` as define in <Add by TuanAnh>                                             \
     * <https://datatracker.ietf.org/doc/html/rfc2326#section-10.7>.           \
     */                                                                        \
    vfunc99(                                                                   \
    ...
    vfunc99(                                                                   \
        ...
        const SmolRTSP_Request *req)
// Định nghĩa các con trỏ hàm cho các phương thức trong giao diện SmolRTSP_Controller
typedef void (*PauseFunc)(SmolRTSP_Context *ctx, const SmolRTSP_Request *req);

typedef struct {
    PauseFunc pause;
    // Thêm các con trỏ hàm cho các phương thức khác nếu cần
} ControllerFuncs;
