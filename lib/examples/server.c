    ...
    */
    ...
    static void Client_pause(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req);
    ...
    static void listener_cb(struct evconnlistener *listener, evutil_socket_t fd, struct sockaddr *sa, int socklen, void *arg) {
        ...
        for (size_t i = 0; i < MAX_STREAMS; i++) {
            client->streams[i].paused = false;
            client->streams[i].playing = false;
        }
        ...
        void *ctx = smolrtsp_libevent_ctx(controller);
            if (!ctx) {
                fprintf(stderr, "Error obtaining context\n");
                free(client);
                bufferevent_free(bev);
                return;
        }
        ...
        // bufferevent_setwatermark(bev, EV_READ, 0, sizeof(SmolRTSP_RequestLine));
        // bufferevent_setwatermark_cb(bev, on_request_line_read, client);
    }
    ...
    static void Client_options(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
        ...
        smolrtsp_header(
            ctx, SMOLRTSP_HEADER_PUBLIC, "DESCRIBE, SETUP, PLAY, PAUSE, TEARDOWN");
    ...
    // }
    static size_t active_seassions = 0;

    static void Client_play(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
        VSELF(Client);

        uint64_t session_id;
        if (smolrtsp_scanf_header(&req->header_map, SMOLRTSP_HEADER_SESSION, "%" SCNu64, &session_id) != 1) {
            smolrtsp_respond(ctx, SMOLRTSP_STATUS_BAD_REQUEST, "Malformed `Session'");
            return;
        }
        ...
        if (!played) {
            smolrtsp_respond(ctx, SMOLRTSP_STATUS_SESSION_NOT_FOUND, "Invalid Session ID");
            return;
        }
    ...
    static void Client_pause(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
        ...
        for (size_t i = 0; i < MAX_STREAMS; i++) {
            if (self->streams[i].session_id == session_id) {
                // Kiểm tra xem stream đang được phát hay không
                if (self->streams[i].playing) {
                    // Drop the stream context
                    VCALL(self->streams[i].ctx, drop);
                    self->streams[i].paused = true; // Set the pause flag
                    paused = true;
                    break;
                } else {
                    // Nếu stream không được phát, trả về lỗi
                    smolrtsp_respond(ctx, SMOLRTSP_STATUS_BAD_REQUEST, "Stream is not active or already paused");
                    return;
                }
            }
        }
        ...
        if (!paused) {
            smolrtsp_respond(ctx, SMOLRTSP_STATUS_SESSION_NOT_FOUND, "Invalid Session ID or Stream is not active");
            return;
        }
    ...
    static void Client_teardown(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
        ...
        for (size_t i = 0; i < MAX_STREAMS; i++) {
            if (self->streams[i].session_id == session_id) {
                // if(self->streams[i].ctx.vptr){
                //     evbuffer_free(self->streams[i].ctx);
                // }
                event_del(self->streams[i].ev);
                teardowned = true;
                active_seassions--; // Decrease the active session count
                break;
            }
        }
        ...
        // If no active sessions, exit event loop
        if (active_seassions == 0) {
    ...
    static SmolRTSP_ControlFlow Client_before(VSelf, SmolRTSP_Context *ctx, const SmolRTSP_Request *req) {
        VSELF(Client);
        ...
        printf(
            ...
            //Add
        CharSlice99 pause_command = CharSlice99_from_str("PAUSE");
        if (CharSlice99_eq(req->start_line.method, pause_command,strcmp)) {
            Client_pause(self, ctx, req);
            return SmolRTSP_ControlFlow_Break;
        }
    ...
    static SmolRTSP_Droppable play_video(
        struct event_base *base, struct bufferevent *bev, SmolRTSP_RtpTransport *t,
        struct event **ev, int *streams_playing) {
        U8Slice99 video = Slice99_typed_from_array(___media_video_h264);//__media_video_h264
